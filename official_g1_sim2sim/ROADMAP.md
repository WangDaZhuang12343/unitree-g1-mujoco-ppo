...
| `visualization/` | Pipeline Debug Viewer | Priority 2 待开发 |
...
1. **Priority 2：Visualization**：显示 Depth Image、Point Cloud、Occupancy Costmap、DWA 候选路径、选中路径、Robot Pose 和 Goal。
2. **Priority 3：Ground Segmentation**：使用 RANSAC/Least Square/Gravity Constraint 替换 MuJoCo geom ID，并用相同 Benchmark 做回归。
...
- Debug Viewer 能在同一时间轴观察深度图、点云、代价地图和规划路径。
- 界面关闭不影响无界面 Benchmark。
- Debug 数据可按开关禁用，禁用时不改变导航结果。
- 优先定位当前 167～279 ms 规划耗时的主要成本，不在未测量前盲目优化。
